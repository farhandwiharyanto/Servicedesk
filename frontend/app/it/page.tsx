...
async function getDashboardData() {
  try {
    const [dashboardData, lookups] = await Promise.all([
      apiFetch(endpoints.dashboard),
      apiFetch(endpoints.lookups),
    ]);
    
    return { 
      total: dashboardData.stats.total_requests,
      open: dashboardData.stats.open_requests,
      inProgress: dashboardData.stats.in_progress_requests,
      overdueCount: dashboardData.stats.overdue_requests,
      recent: dashboardData.recent_activity.latest_requests,
    ...
    console.warn("API fetch failed in Dashboard. Using fallback data.");
    return {
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: '24px' }}>
        <div className="card" style={{ padding: '0' }}>
          <div style={{ padding: '20px', borderBottom: '1px solid var(--border-subtle)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <Link href="/it/requests" style={{ fontSize: '13px', color: 'var(--primary)', fontWeight: 600 }}>View All</Link>
          </div>
