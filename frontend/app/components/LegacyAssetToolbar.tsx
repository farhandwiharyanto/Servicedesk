...
interface LegacyAssetToolbarProps {
  onNew: () => void;
  selectedCount: number;
}

export function LegacyAssetToolbar({ onNew, selectedCount }: LegacyAssetToolbarProps) {
  return (
    <div className="legacy-btn-toolbar" style={{ border: '1px solid #d4d4d4', borderBottom: 'none', background: '#fdfdfd' }}>
      <button className="legacy-btn" style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
         🖥️ All Assets <span style={{ fontSize: '8px' }}>▼</span>
      </button>
      ...
      <button className="legacy-btn">📡 Scan</button>
      <button className="legacy-btn">🔄</button>
