...
interface LegacyProblemToolbarProps {
  onNew: () => void;
  selectedCount: number;
}

export function LegacyProblemToolbar({ onNew, selectedCount }: LegacyProblemToolbarProps) {
  return (
    <div className="legacy-btn-toolbar" style={{ border: '1px solid #d4d4d4', borderBottom: 'none', background: '#fdfdfd' }}>
      <button className="legacy-btn" style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
         🔥 All Problems <span style={{ fontSize: '8px' }}>▼</span>
      </button>
      ...
      <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '11px' }}>
        ...
        <button className="legacy-btn" style={{ border: '1px solid #ccc', borderRadius: '2px', marginLeft: '8px' }}>☰</button>
      </div>
    </div>
  );
}
